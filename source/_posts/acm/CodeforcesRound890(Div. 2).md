...
读题易得：若原来的值是 $1$，那么必须找别的值借 $1$ 才能保证 $a_i \nq b_i$，而其他值则都可以简单变成 $1$ 解决。故只需要计算有多少可以冗余调配的值即可
...
如此递归下去，可以得到最终的查询费用为 $2(n - 1)^2 + 2 \times 2(0.5n - 1)^2 + 2 \times 4(0.25n - 1)^2 + \dots \leq 2n^2 + 4(0.5n)^2 +  8(0.25n)^2 + \dots = 2(n^2 + 0.5n^2 + 0.25n^2 + \dots) \leq 4n^2 \leq 5n^2$
